...
namespace irr
{
	namespace scene
	{
		...
		//! constructor
		FSSceneNodeAnimatorFPS::FSSceneNodeAnimatorFPS(gui::ICursorControl* cursorControl,
		...
		//! destructor
		FSSceneNodeAnimatorFPS::~FSSceneNodeAnimatorFPS()
		{
			if (CursorControl)
				CursorControl->drop();
		}
		...
		bool FSSceneNodeAnimatorFPS::OnEvent(const SEvent& evt)
		{
		...
		void FSSceneNodeAnimatorFPS::animateNode(ISceneNode* node, u32 timeMs)
		{
		...
		void FSSceneNodeAnimatorFPS::allKeysUp()
		{
		...
		void FSSceneNodeAnimatorFPS::setRotateSpeed(f32 speed)
		{
			RotateSpeed = speed;
		}
		...
		void FSSceneNodeAnimatorFPS::setMoveSpeed(f32 speed)
		{
			MoveSpeed = speed;
		}
		...
		f32 FSSceneNodeAnimatorFPS::getRotateSpeed() const
		{
			return RotateSpeed;
		}
		...
		f32 FSSceneNodeAnimatorFPS::getMoveSpeed() const
		{
			return MoveSpeed;
		}
		...
		void FSSceneNodeAnimatorFPS::setKeyMap(SKeyMap *map, u32 count)
		{
			// clear the keymap
			KeyMap.clear();
		...
		void FSSceneNodeAnimatorFPS::setKeyMap(const core::array<SKeyMap>& keymap)
		{
			KeyMap = keymap;
		}

		const core::array<SKeyMap>& FSSceneNodeAnimatorFPS::getKeyMap() const
		{
			return KeyMap;
		}
		...
		void FSSceneNodeAnimatorFPS::setVerticalMovement(bool allow)
		{
		...
		void FSSceneNodeAnimatorFPS::setInvertMouse(bool invert)
		{
			if (invert)
		...
		ISceneNodeAnimator* FSSceneNodeAnimatorFPS::createClone(ISceneNode* node, ISceneManager* newManager)
		{
			FSSceneNodeAnimatorFPS * newAnimator =
				new FSSceneNodeAnimatorFPS(CursorControl, RotateSpeed, MoveSpeed, JumpSpeed,

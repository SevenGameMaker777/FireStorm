...
{
	//! constructor
	FSGUI_Menu::FSGUI_Menu(IrrlichtDevice* device, IGUIEnvironment* environment, IGUIElement* parent,
			s32 id, core::rect<s32> rectangle)
		: CGUIContextMenu(environment, parent, id, rectangle, false, true)
	{
		#ifdef _DEBUG
		setDebugName("FSGUI_Menu");
		#endif
...
//		skin->draw3DToolBar(this, rect, &AbsoluteClippingRect);
	...
	bool FSGUI_Menu::OnEvent(const SEvent& event)
	{
		if (isEnabled())
		{
					...
					if (!mouseClicked)
					{
						...
						for (u32 i = 0; i < Items.size(); ++i)
							if (Items[i].SubMenu && Items[i].SubMenu->isVisible())
								opensubmenu = i;
						if ((opensubmenu != -1) && (Items[opensubmenu].SubMenu->getAbsolutePosition().isPointInside(p)))
							return true;
					}
	...
	void FSGUI_Menu::recalculateSize()
	{
	...
	core::rect<s32> FSGUI_Menu::getHRect(const SItem& i, const core::rect<s32>& absolute) const
	{
	...
	core::rect<s32> FSGUI_Menu::getRect(const SItem& i, const core::rect<s32>& absolute) const
	{
		return getHRect(i, absolute);
	}


	void FSGUI_Menu::updateAbsolutePosition()
	{
		if (Parent)

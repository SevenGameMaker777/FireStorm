...
namespace irr
{
	namespace gui
	{
		...
		//! constructor
		FSGUI_Combobox::FSGUI_Combobox(IrrlichtDevice* device, IGUIEnvironment* environment, IGUIElement* parent,
		...
		void FSGUI_Combobox::setTextAlignment(EGUI_ALIGNMENT horizontal, EGUI_ALIGNMENT vertical)
		{
			HAlign = horizontal;
			VAlign = vertical;
		...
		void FSGUI_Combobox::setMaxSelectionRows(u32 max)
		{
		...
		u32 FSGUI_Combobox::getMaxSelectionRows() const
		{
		...
		u32 FSGUI_Combobox::getItemCount() const
		{
			return Items.size();
		}
		...
		const wchar_t* FSGUI_Combobox::getItem(u32 idx) const
		{
			if (idx >= Items.size())
				return 0;
		...
		//! returns string of an item. the idx may be a value from 0 to itemCount-1
		u32 FSGUI_Combobox::getItemData(u32 idx) const
		{
			if (idx >= Items.size())
				return 0;
		...
		s32 FSGUI_Combobox::getIndexForItemData(u32 data) const
		{
		...
		void FSGUI_Combobox::removeItem(u32 idx)
		{
			if (idx >= Items.size())
				return;
		...
		const wchar_t* FSGUI_Combobox::getText() const
		{
		...
		u32 FSGUI_Combobox::addItem(const wchar_t* text, u32 data)
		{
		...
		s32 FSGUI_Combobox::getSelected() const
		{
			return Selected;
		}
		...
		void FSGUI_Combobox::setSelected(s32 idx)
		{
		...
		bool FSGUI_Combobox::OnEvent(const SEvent& event)
		{
			if (isEnabled())
			{
				...
				case EET_GUI_EVENT:
					...
					{
						//seven
						core::position2d<s32> p(m_Device->getCursorControl()->getPosition());
						if (ListBox && ListBox->isPointInside(p)) return true;
		...
		void FSGUI_Combobox::sendSelectionChangedEvent()
		{
			if (Parent)
			{
				SEvent event;
		...
		void FSGUI_Combobox::openCloseMenu()
		{
			if (ListBox)
			{
				// close list box
		...
		void FSGUI_Combobox::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options = 0) const
		{
		...
		void FSGUI_Combobox::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options = 0)
		{

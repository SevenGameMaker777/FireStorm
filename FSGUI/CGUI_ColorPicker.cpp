...
namespace irr
{
	...
	namespace gui
	{
		IGUIColorPicker::IGUIColorPicker(IGUIEnvironment *environment, IGUIElement *parent, s32 id, video::SColor v, bool uad)
...
#undef implement_scrollbar
			...
			setPickedColor(v,false);
			setSelectedColor(v,false);
			updateAbsolutePosition();
		}
		IGUIColorPicker::~IGUIColorPicker() 
		{
			close->drop();
			extend->drop();
			scroll->drop();
		...
		IGUIButton *IGUIColorPicker::getCloseButton() const 
		{
			return close;
		}

			IGUIButton *IGUIColorPicker::getExtendButton() const 
		{
			return extend;
		}
			void IGUIColorPicker::createAlphaTexture() 
		{
...
#undef square
			...
			void IGUIColorPicker::createGradientTexture() 
		{
...
#undef interpolate
		...
		void IGUIColorPicker::setAdvenced(bool i) 
		{
			IGUISkin *skin = Environment->getSkin();
			isExtended = i;
			...
			bool IGUIColorPicker::getAdvenced() const 
		{
			return isExtended;
		}
			bool IGUIColorPicker::OnEvent(const SEvent &event) 
		{
//			if (!isEnabled())
...
*/					}
			...
			void IGUIColorPicker::setScrollValueHSV(bool set, int h, int s, int v) 
		{
			core::stringw value;
...
#undef set
			...
			void IGUIColorPicker::setScrollValueRGB(bool set, int r, int g, int b) 
		{
			core::stringw value;
...
#undef set
			...
			void IGUIColorPicker::updateFromScroll(bool HsvOrRgb) 
		{
			if (HsvOrRgb)
			{
			...
			void IGUIColorPicker::recalculateAdvenced() 
		{
			if (!isExtended)
			return;
			...
			void IGUIColorPicker::recalculatePickedColor(bool sendevent)
		{
			u32 *tmp = (u32*)img[1]->lock();
			...
			//setPickedColor(getPickedColor());
			setSelectedColor(pickcolor,sendevent);
			}
			void IGUIColorPicker::setPickedColor(const video::SColor &c, bool sendevent)
		{
			core::vector3df hsv = RGBftoHSV({
				c.getRed() / 255.f,
				c.getGreen() / 255.f,
			...
			recalculatePickedColor(sendevent);

			setScrollValueHSV(true, hsv.X, hsv.Z * 100, hsv.Y * 100);
			setScrollValueRGB(true, c.getRed(), c.getGreen(), c.getBlue());

			setSelectedColor(getPickedColor(),sendevent);

		}
			const video::SColor& IGUIColorPicker::getPickedColor() const 
		{
			return pickcolor;
		}
			void IGUIColorPicker::setSelectedColor(const video::SColor &b, bool sendevent)
			{
				selectcolor = b;

				printf("setting color %d %d %d %d\n", selectcolor.getAlpha(), selectcolor.getRed(), selectcolor.getGreen(), selectcolor.getGreen());
				...
				if (sendevent)
				{
					...
					event.GUIEvent.Caller = this;
					event.GUIEvent.Element = 0;
					event.GUIEvent.EventType = EGET_TABLE_CHANGED; // EGET_FILE_SELECTED;
					Parent->OnEvent(event);
				}
			}

			const video::SColor& IGUIColorPicker::getSelectedColor() const 
		{
			return selectcolor;
		}
			void IGUIColorPicker::setBackgroundColor(const video::SColor &b) 
		{
			background = b;
		}
			const video::SColor& IGUIColorPicker::getBackgroundColor() const 
		{
			return background;
		}
		...
		{
			if (!isVisible()) return;
			if (getAbsolutePosition().LowerRightCorner.Y > getParent()->getAbsoluteClippingRect().LowerRightCorner.Y) return;

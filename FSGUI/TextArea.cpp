...
//				changeStartIndex((s32)event.MouseInput.Wheel);
				changeStartIndex(scrollBar->getPos());
				return true;
			}
		}
	}
...
const std::vector<Line*>& TextArea::getLines() {
	return lines;
}
...
// seven
	if (scrollBar) scrollBar->setMax(lines.size());
...
//Seven		const core::vector2di upperLeftCorner = RelativeRect.UpperLeftCorner - borderSize;
		const core::vector2di upperLeftCorner = getAbsolutePosition().UpperLeftCorner - borderSize;
...
void TextArea::updateScrollBar() {
	core::vector2di scrollBarPosition = core::vector2di(RelativeRect.getWidth() - scrollBarWidth, 2);

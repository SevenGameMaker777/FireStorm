    ...
    : ISceneNode( parent, manager, id, position, rotation, scale )
{
	...
	BillboardMaterial.Lighting = true;
	BillboardMaterial.FogEnable = true;
...
void CTreeSceneNode::OnRegisterSceneNode()
{
    if ( IsVisible )
    {
        ...
        if ( LeafNode )
        {
            f32 far1 = FarRange + Size;
            LeafNode->setVisible( far1*far1 >= DistSQ );
        }
...
void CTreeSceneNode::render()
{
    ...
    f32 far1 = FarRange + Size;
    f32 mid1 = MidRange + Size;
    
	if ( far1*far1 < DistSQ && BillboardMaterial.TextureLayer[0].Texture != 0 )
    {
    ...
    if ( mid1*mid1 < DistSQ && MidLODMeshBuffer != 0 )
    {
